...
class FeedStatStype(Base2):
    __tablename__ = "feed_stat_stype"

    date = Column(types.Date, primary_key=True)
    stype = Column(types.BigInteger, primary_key=True)
    origin = Column(types.String(128), primary_key=True)
    ...
    def __init__(date, stype, origin, dispatch, tosize, dispatch_user, view, view_pos, view_user, viewed_user, reply, reply_user, replyed_user, click, click_pos, click_user, clicked_user):
        self.date = date
        self.stype = stype
        self.origin = origin
    ...
    def __repr__(self):
        return "<FeedStatStype('%s')" % self.stype
