...
class FeedStatUseridStype(Base2):
    __tablename__ = "feed_stat_userid_stype"

    date = Column(types.Date, primary_key=True)
    userid = Column(types.Integer, primary_key=True)
    stype = Column(types.BigInteger, primary_key=True)
    origin = Column(types.String(128))
    ...
    def __init__(date, userid, stype, origin, dispatch, tosize, dispatch_user, view, view_pos, view_user, viewed_user, reply, reply_user, replyed_user, click, click_pos, click_user, clicked_user):
        self.date = date
        self.userid = userid
        self.stype = stype
        self.origin = origin
